/* Good-Loop Video Adverts AdUnit

This file should work on mobile & desktop, and on moderately old browsers.
It must be small (when minified) and load fast.

Assumes: jQuery or Zepto!!!
Insert into the file in production.
...
var variant = {
	...
	mobilectaheadline: '<span style="color:royalblue;">Ads For Good:<br>Donate by watching 1 advert</span>',
	ctaheadline: 
	// "Donate {variant.adsecs} seconds of your time by watching an ad."
	// Choose a charity and donate for free by watching an advert'
	'<span style="color:royalblue;">Ads For Good: Give a donation<br>by watching an advert</span>'
	// "Give a charity donation<br>By watching an advert"
};

var adSecs = variant.adsecs;
// Thank You (and no repeat) valid across this site for 1 minute TODO 1 day
var expires = 1.0 / (24*60);
...
var $styles = $(
'<style>' +
'.goodloop-adunit, .goodloop-vidbox {' +
'}' +
'.goodloop-vidbox a {' +
'	transition: none;' +
'	color: white;' +
...
'	border-radius: 3px;' +
'	box-shadow: inset 1px -1px 1px 0px rgba(127,127,127,0.5);' +
'	margin: 0px;' +
'	padding: 2px;' +
'	margin-top: 3px;' +
'}' +
'.goodloop-charitylogowide {' +
'	max-width: 230px;' +
'	max-height: 43px;' +
'	margin: 0px;' +
'	padding: 0px;' +
'}' +
'.goodloop-playoverlay {' +
'	position: absolute;' +
'	top: 0;' +
'	left: 0;' +
'	width: 100%;' +
...
if (window.goodloop.pick) return;
...
function renderUnit(id, $div) {
	...
	var cname = Cookies.get('charity');
	if (cname) {
		var tqhtml = tqSideRectangle({width:300, height:250, adunitid:id, charity:cname});
		$div.html(tqhtml);
		return;	
	}

	if (isMobile()) {
		$('body').append($(adFixedBottom($(window).width(), 100, id)));
	} else {
		$div.html(adSideRectangle(300,250,id));
	}
}
// }, 5);

var pickedCharity = null, startTime = null;
...
/// AD-UNITS
// TODO use webpack / babel and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals

const VIDBOX =
'<div class="goodloop-vidbox">'	+
'	<center style="margin:0px;padding:0px;">' +
'		<div class="adcontents"></div>' +
'	</center>' +
'</div>';
...
function adSideRectangle(w, h, adunitid) {
	...
	for(var ci=0; ci<charities.length; ci++) {
		...
		if ( ! charity) {
			console.warn("no charity?!", charities, goodloop.unit);
			continue;
		}
		myunit +=
...
function adFixedBottom(w, h, adunitid) {
	...
	for(var ci = 0; ci < charities.length; ci++) {
		var charity = charities[ci];
		myunit +=
	'		<div style="width:30%;vertical-align:top;margin-left:2.5%;padding:0px;display:inline-block;">' +
	'			<div title="' + charity.name + '" data-charity="' + charity.name + '" data-charity-url="' + charity.url + '" onclick="goodloop.pick(this);" class="goodloop-play">' +
	'				<center style="margin:0px;padding:0px;max-height:43px;">' +
	'					<img src="' + charity.logo + '" style="max-width:100%; max-height:43px;margin:0px;padding:0px;">' +
	'				</center>' +
	'			</div>' +
	'		</div>';
	}
	...
	myunit +=
	'	</div>' +
	'	<div style="position:absolute;top:5px;right:5px;font-size:18px;" onclick="goodloop.hideUnit();">×</div>' +
	'</div>';
	return myunit;
}
...
// don't drop stuff!
goodloop.pick = pick;
goodloop.clickthru = clickthru;
goodloop.skip = skip;
goodloop.hideUnit = hideUnit;
